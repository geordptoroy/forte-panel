...
function normalizeBaileysInbound(event: any): InboundMessageEvent {
  const normalized = normalizePapiInbound(event);
  return {
    ...normalized,
...
export function createPapiAdapter(): PapiAdapter {
  const baseUrl =
    ENV.papiDeployment === "cloud"
      ? ENV.papiCloudApiUrl
      : process.env.PAPI_BASE_URL;
  ...
  return {
    ...
    async health() {
      if (!baseUrl)
        return nowHealth("papi", false, "PAPI base URL não configurada");
      const started = Date.now();
      try {
        const response = await fetch(`${baseUrl.replace(/\/$/, "")}/health`, {
          headers: fallbackApiKey ? { "x-api-key": fallbackApiKey } : undefined,
        });
        return nowHealth(
          "papi",
          response.ok,
          response.ok ? "PAPI disponível" : `PAPI respondeu ${response.status}`,
          Date.now() - started
        );
      } catch (error) {
        return nowHealth(
    ...
    async sendMessage(command: OutboundMessageCommand) {
      ...
      const resolvedInstanceId =
        command.instanceId ?? process.env.PAPI_INSTANCE_ID;
      if (!resolvedInstanceId)
        throw new Error(
          "PAPI instanceId não informado (envie instanceId ou configure PAPI_INSTANCE_ID)"
        );
      ...
      if (messageType === "audio") {
        ...
        payload = {
          ...
          buttons,
          headerType:
            typeof metadata.headerType === "string"
              ? metadata.headerType
              : "none",
        };
        ...
        throw new Error(
          `Tipo de mensagem ${messageType} ainda não é suportado pelo adapter PAPI`
        );
      }
  ...
  return {
    ...
    async health() {
      if (!accessToken || !phoneNumberId)
        return nowHealth(
          "meta_cloud_api",
          false,
          "META_WHATSAPP_ACCESS_TOKEN ou META_WHATSAPP_PHONE_NUMBER_ID não configurado"
        );
      const started = Date.now();
      try {
    ...
    async sendMessage(command: OutboundMessageCommand) {
      ...
      if (command.messageType && command.messageType !== "text")
        throw new Error(
          `WhatsApp Cloud API ainda não suporta o tipo ${command.messageType} neste worker`
        );
      const response = await fetch(`${baseUrl}/${phoneNumberId}/messages`, {
        ...
        body: JSON.stringify({
          ...
          to: normalizePhone(command.phone),
          type: "text",
          text: { body: command.content },
        }),
      });
      if (!response.ok)
        throw new Error(`Meta Cloud API respondeu ${response.status}`);
      const data = (await response.json()) as {
        messages?: Array<{ id?: string }>;
      };
      return {
        externalId: String(data.messages?.[0]?.id ?? command.idempotencyKey),
        status: "sent" as const,
      };
    },
