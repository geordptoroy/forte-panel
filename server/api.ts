...
const messageSchema = z
  ...
  .refine(input => Boolean(input.contactId || input.phone), {
    message: "contactId ou phone é obrigatório",
  })
  .refine(
    input =>
...
function requireApiKey(req: Request, res: Response) {
  ...
  if (!expected) {
    fail(
      res,
      503,
      "FORTE_API_KEY não configurada no servidor",
      "api_not_configured"
    );
    return false;
  }
...
async function requireApiWorkspaceId(
  res: Response
  ...
  if (!Number.isSafeInteger(workspaceId) || workspaceId <= 0) {
    fail(
      res,
      503,
      "FORTE_API_WORKSPACE_ID não configurado para a API REST",
      "api_workspace_not_configured"
    );
    return undefined;
  }
  ...
  if (!workspace) {
    fail(
      res,
      503,
      "FORTE_API_WORKSPACE_ID não corresponde a um workspace ativo",
      "api_workspace_not_configured"
    );
    return undefined;
  }
...
async function idempotent(
  ...
  if (claim.inProgress) {
    ...
    return fail(
      res,
      409,
      "Já existe uma requisição em processamento para esta chave",
  ...
  try {
    ...
    if (normalized.phone.length < 8 || !normalized.content.trim())
      return fail(
        res,
        400,
        "Evento Baileys sem telefone ou conteúdo",
        "invalid_payload"
      );
    const registered = await registerWebhookEvent({
      eventId,
    ...
    return fail(
      ...
      error instanceof Error
        ? error.message
        : "Falha ao processar webhook Baileys",
      "internal_error"
    );
  }
}
