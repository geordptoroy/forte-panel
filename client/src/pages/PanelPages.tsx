...
function MessageBubble({ message }: { message: Message }) {
  ...
  const mediaUrlValue = typeof metadata.mediaUrl === "string" ? metadata.mediaUrl : message.messageType === "audio" ? message.text : "";
  const safeMediaUrl = (() => { try { const url = new URL(mediaUrlValue); return url.protocol === "https:" || url.protocol === "http:" ? url.href : ""; } catch { return ""; } })();
  const buttons = Array.isArray(metadata.buttons) ? metadata.buttons as Array<{ id?: string; displayText?: string }> : [];
  const mediaLabel = typeof metadata.fileName === "string" && metadata.fileName ? metadata.fileName : message.messageType === "image" ? "Imagem recebida" : message.messageType === "video" ? "Vídeo recebido" : "Documento recebido";
  return <div className={`message-row from-${message.sender}`}><div className="message-bubble"><div className="message-author">{author}</div><div className="message-text">{message.messageType === "audio" && safeMediaUrl ? <audio controls preload="none" src={safeMediaUrl} aria-label="Mensagem de áudio" /> : message.messageType && ["image", "video", "document"].includes(message.messageType) ? safeMediaUrl ? <a href={safeMediaUrl} target="_blank" rel="noreferrer">{mediaLabel}</a> : <span>{message.text || `[${mediaLabel}]`}</span> : message.text}{buttons.length > 0 && <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>{buttons.map((button, index) => <span key={`${button.id ?? "option"}-${index}`} className="btn-secondary" style={{ cursor: "default", padding: "4px 8px" }}>{button.displayText || button.id || `Opção ${index + 1}`}</span>)}</div>}{message.messageType === "button" && typeof metadata.buttonText === "string" && <div>{metadata.buttonText}</div>}</div><div className="message-time">{time} {message.sender !== "system" && <Check size={10} style={{ display: "inline", verticalAlign: "middle" }} />}</div></div></div>;
}
...
export function InboxPage() {
  ...
  return <PanelLayout eyebrow="Operação / Atendimento" title="Atendimento" description="Converse com seus clientes sem sair do painel." actions={<PageLink href="/contacts" className="btn-primary"><Plus size={13} /> Nova conversa</PageLink>}>
    <DemoBanner />
...
export function AgendaPage() {
  ...
  const statusLabel = (status: string) => status === "confirmed" ? "Confirmado" : status === "requested" ? "Solicitado" : status === "in_progress" ? "Em andamento" : status === "completed" ? "Concluído" : status === "cancelled" ? "Cancelado" : "Não compareceu";
  const remoteAppointments = (agendaQuery.data?.appointments ?? []).map((item) => { const startsAt = new Date(item.startsAt); const endsAt = new Date(item.endsAt); return { id: String(item.id), contactName: item.contactName ?? "Cliente sem nome", professionalName: item.professionalName ?? "Profissional", service: item.serviceName ?? "Atendimento", date: startsAt.toLocaleDateString("pt-BR", { timeZone: workspaceTimezone }), time: startsAt.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit", timeZone: workspaceTimezone }), duration: `${Math.max(15, Math.round((endsAt.getTime() - startsAt.getTime()) / 60000))} min`, status: statusLabel(item.status), rawStatus: item.status, notes: item.notes ?? "" }; });
...
export function IntegrationsPage() {
  ...
  const papiConfigQuery = trpc.workspace.papiConfig.useQuery();
  const papiInstancesQuery = trpc.workspace.papiInstances.useQuery();
  const papiCloudStatusQuery = trpc.workspace.papiCloudStatus.useQuery();
  const usageQuery = trpc.workspace.usage.useQuery(undefined, { refetchInterval: 30_000 });
  const [savingProvider, setSavingProvider] = useState<string | null>(null);
  const [instanceId, setInstanceId] = useState("");
  const [cloudInstanceName, setCloudInstanceName] = useState("");
  const [webhookName, setWebhookName] = useState("");
  const [copied, setCopied] = useState<string | null>(null);
  const [newWebhookSecret, setNewWebhookSecret] = useState<{ name: string; secret: string } | null>(null);
  const createPapiCloudInstanceMutation = trpc.workspace.createPapiCloudInstance.useMutation({ onSuccess: async (created) => { setNewWebhookSecret({ name: `${created.name} · ${created.instanceId}`, secret: created.webhookSecret ?? "" }); setCloudInstanceName(""); await Promise.all([papiConfigQuery.refetch(), papiInstancesQuery.refetch()]); } });
  ...
  const createPapiWebhookMutation = trpc.workspace.createPapiWebhook.useMutation({ onSuccess: async (created) => { if (created.secret) setNewWebhookSecret({ name: created.name, secret: created.secret }); setInstanceId(""); setWebhookName(""); await Promise.all([papiConfigQuery.refetch(), papiInstancesQuery.refetch()]); }, });
  const setDefaultPapiWebhookMutation = trpc.workspace.setDefaultPapiWebhook.useMutation({ onSuccess: async () => { await Promise.all([papiConfigQuery.refetch(), papiInstancesQuery.refetch()]); } });
  const setDefaultPapiInstanceMutation = trpc.workspace.setDefaultPapiInstance.useMutation({ onSuccess: async () => { await Promise.all([papiConfigQuery.refetch(), papiInstancesQuery.refetch()]); } });
  const deletePapiWebhookMutation = trpc.workspace.deletePapiWebhook.useMutation({ onSuccess: async () => { await Promise.all([papiConfigQuery.refetch(), papiInstancesQuery.refetch()]); } });
  const channels = channelsQuery.data ?? [];
  const papiConfig = papiConfigQuery.data;
  const papiWebhooks = papiConfig?.webhooks ?? [];
  const papiInstances = papiInstancesQuery.data ?? [];
  const usage = usageQuery.data;
  const usageMetrics = usage ? ([
    ["apiRequests", "API"],
    ["aiRequests", "Execuções de IA"],
    ["outboundMessages", "Mensagens outbound"],
  ] as const).map(([key, label]) => ({ key, label, ...usage.workspace[key] })) : [];
  ...
  const copyValue = async (key: string, value: string) => { await navigator.clipboard.writeText(value); setCopied(key); window.setTimeout(() => setCopied(null), 1800); };
  return <PanelLayout eyebrow="Sistema / Conectividade" title="Integrações" description="Escolha o canal de WhatsApp e associe a instância PAPI que receberá e enviará as mensagens."><DemoBanner /><section className="surface" style={{ marginTop: 20, padding: 20 }}><SectionTitle eyebrow="Consumo do workspace" title={usage ? `Plano ${usage.plan}` : "Cotas e consumo"} action={<StatusBadge tone={usageQuery.isError ? "red" : "green"}>{usageQuery.isFetching ? "Atualizando" : "Ao vivo"}</StatusBadge>} />{usageQuery.isLoading ? <div className="muted">Carregando consumo atual...</div> : usageQuery.error ? <div className="form-error">{usageQuery.error.message}</div> : usage ? <><p className="muted">Janela atual até {new Date(usage.resetsAt).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}. Os limites são isolados por workspace e operador.</p><div className="stat-grid" style={{ marginTop: 16 }}>{usageMetrics.map((metric) => { const percent = usagePercent(metric.used, metric.limit); return <div className="surface surface-hover stat-card" key={metric.key}><div className="stat-top"><span className="stat-label">{metric.label}</span><Gauge size={16} className={percent >= 80 ? "amber" : "green"} /></div><strong className="stat-value">{metric.used} <small style={{ fontSize: 12, fontWeight: 500 }}>/ {metric.limit}</small></strong><span className="stat-foot">{Math.max(0, metric.limit - metric.used)} restantes · {percent}% usado</span><div style={{ height: 5, background: "rgba(0,0,0,.08)", borderRadius: 99, marginTop: 10 }}><div style={{ width: `${percent}%`, height: "100%", background: percent >= 80 ? "#d28a24" : "#2d8a61", borderRadius: 99 }} /></div></div>; })}</div><div style={{ marginTop: 20 }}><SectionTitle eyebrow="Operadores" title={`${usage.users.length} operadores com consumo na janela`} />{usage.users.length === 0 ? <div className="muted">Nenhum consumo individual registrado nesta janela.</div> : <div className="list-stack">{usage.users.slice().sort((a, b) => b.usage.outboundMessages.used - a.usage.outboundMessages.used).map((user) => <div className="profile-field" key={user.userId}><span><strong>{user.name || "Operador sem nome"}</strong><small>{user.email || "Sem e-mail"} · {user.active ? "Ativo" : "Inativo"}</small></span><span style={{ textAlign: "right" }}><strong>{user.usage.outboundMessages.used}/{user.usage.outboundMessages.limit}</strong><small>mensagens outbound</small></span></div>)}</div>}</div></> : null}</section><section className="surface channel-selector-panel"><SectionTitle eyebrow="Canal de atendimento" title="WhatsApp padrão" action={<StatusBadge tone={channels.find((channel) => channel.provider === (defaultChannelQuery.data ?? "papi"))?.configured ? "green" : "amber"}>{channels.find((channel) => channel.provider === (defaultChannelQuery.data ?? "papi"))?.configured ? providerLabel(defaultChannelQuery.data ?? "papi") : "Credencial pendente"}</StatusBadge>} /><p className="muted channel-selector-copy">O CRM pode operar com PAPI, com a API oficial da Meta ou com os dois. A escolha abaixo será usada quando uma mensagem não informar um provedor específico.</p><div className="channel-grid">{channels.map((channel) => { const selected = defaultChannelQuery.data === channel.provider; return <button type="button" key={channel.id} disabled={!channel.configured || setDefaultMutation.isPending} className={`channel-option ${selected ? "is-selected" : ""}`} onClick={() => { setSavingProvider(channel.provider); setDefaultMutation.mutate({ provider: channel.provider as "papi" | "meta_cloud_api" }); }}><div className="channel-option-top"><div className="integration-icon"><MessageCircle size={16} /></div><StatusBadge tone={selected ? "green" : channel.configured ? "neutral" : "amber"}>{selected ? "Padrão" : channel.configured ? "Configurado" : "Pendente"}</StatusBadge></div><strong>{providerLabel(channel.provider)}</strong><small>{channel.provider === "meta_cloud_api" ? "Token e Phone Number ID da Meta ficam apenas no servidor." : "Canal conectado pela API Pastorini, isolado por adapter."}</small>{savingProvider === channel.provider && <span className="channel-saving">Salvando seleção...</span>}</button>; })}</div></section><section className="surface" style={{ marginTop: 20, padding: 20 }}><SectionTitle eyebrow="PAPI Cloud" title="Provisionar instância Cloud" /><p className="muted">Cria uma instância na PAPI Cloud, busca a API key no backend e configura automaticamente o webhook deste workspace. A operação só fica disponível quando a flag de provisionamento estiver habilitada no Compose.</p><div className="form-grid" style={{ marginTop: 16 }}><div className="form-field"><label htmlFor="papi-cloud-instance-name">Nome da instância</label><input id="papi-cloud-instance-name" className="input-control" value={cloudInstanceName} onChange={(event) => setCloudInstanceName(event.target.value)} placeholder="Ex.: Comercial Cloud" /></div><div className="form-field" style={{ alignSelf: "end" }}><button type="button" className="btn-primary" disabled={!papiCloudStatusQuery.data?.provisioningEnabled || !papiCloudStatusQuery.data?.tokenConfigured || createPapiCloudInstanceMutation.isPending || !cloudInstanceName.trim()} onClick={() => createPapiCloudInstanceMutation.mutate({ name: cloudInstanceName })}>{createPapiCloudInstanceMutation.isPending ? "Provisionando..." : "Criar na PAPI Cloud"}</button></div></div><div className="demo-banner" style={{ marginTop: 14, marginBottom: 0 }}>{papiCloudStatusQuery.data?.provisioningEnabled && papiCloudStatusQuery.data?.tokenConfigured ? <><CheckCircle2 size={14} /> Provisionamento habilitado. Deployment atual do Panel: <code>{papiCloudStatusQuery.data.deployment}</code>.</> : <><Info size={14} /> Provisionamento desativado ou sem token no backend. A self-hosted continua sendo usada localmente.</>}</div>{createPapiCloudInstanceMutation.error && <div className="form-error" style={{ marginTop: 12 }}>{createPapiCloudInstanceMutation.error.message}</div>}</section><section className="surface" style={{ marginTop: 20, padding: 20 }}><SectionTitle eyebrow="PAPI / Pastorini" title="Instâncias persistentes" /><p className="muted">Cada instância pertence a este workspace. A API key fica somente no backend e aparece aqui apenas mascarada.</p><div className="list-stack" style={{ marginTop: 16 }}>{papiInstances.length === 0 ? <div className="muted">Nenhuma instância persistida ainda. Criar um webhook abaixo faz o backfill automaticamente.</div> : papiInstances.map((instance) => <article className="surface" style={{ padding: 16 }} key={instance.id}><div className="profile-field"><span><strong>{instance.name}</strong><small className="muted">{instance.instanceId} · {instance.deployment === "cloud" ? "PAPI Cloud" : "Self-hosted"}</small></span><div style={{ display: "flex", gap: 8, alignItems: "center" }}>{instance.isDefault ? <StatusBadge tone="green">Instância padrão</StatusBadge> : <button type="button" className="btn-secondary" disabled={setDefaultPapiInstanceMutation.isPending || !instance.active} onClick={() => setDefaultPapiInstanceMutation.mutate({ id: instance.id })}>Usar nos envios</button>}<StatusBadge tone={instance.active ? instance.status === "error" ? "red" : "green" : "amber"}>{instance.active ? instance.status : "Inativa"}</StatusBadge></div></div><div className="profile-field" style={{ marginTop: 10 }}><span>API key</span><code>{instance.apiKeyMasked || "não cadastrada — usa fallback local"}</code></div>{instance.lastHealthError && <div className="form-error" style={{ marginTop: 10 }}>{instance.lastHealthError}</div>}</article>)}</div></section><section className="surface" style={{ marginTop: 20, padding: 20 }}><SectionTitle eyebrow="PAPI / Pastorini" title="Webhooks por instância" /><p className="muted">Crie um webhook separado para cada instância PAPI. Cada registro gera uma URL e um segredo próprios; assim, os eventos entram já associados à instância correta.</p><div className="form-grid" style={{ marginTop: 16 }}><div className="form-field"><label htmlFor="papi-webhook-name">Nome do webhook</label><input id="papi-webhook-name" className="input-control" value={webhookName} onChange={(event) => setWebhookName(event.target.value)} placeholder="Ex.: Comercial" /></div><div className="form-field"><label htmlFor="papi-instance-id">Instance ID da PAPI</label><input id="papi-instance-id" className="input-control" value={instanceId} onChange={(event) => setInstanceId(event.target.value)} placeholder="Ex.: funil" /></div><div className="form-field" style={{ alignSelf: "end" }}><button type="button" className="btn-primary" disabled={createPapiWebhookMutation.isPending || !webhookName.trim() || !instanceId.trim()} onClick={() => createPapiWebhookMutation.mutate({ name: webhookName, instanceId })}>{createPapiWebhookMutation.isPending ? "Criando..." : "Criar webhook"}</button></div></div>{createPapiWebhookMutation.error && <div className="form-error" style={{ marginTop: 12 }}>{createPapiWebhookMutation.error.message}</div>}<div className="demo-banner" style={{ marginTop: 16, marginBottom: 0 }}>{newWebhookSecret ? <><Info size={14} /> Segredo exibido somente agora para <strong>{newWebhookSecret.name}</strong>. Copie e guarde-o na PAPI; ele não será mostrado novamente nesta tela. <button type="button" className="btn-secondary" onClick={() => copyValue("new-secret", newWebhookSecret.secret)}><Copy size={13} />{copied === "new-secret" ? "Copiado" : "Copiar segredo"}</button><code style={{ wordBreak: "break-all" }}>{newWebhookSecret.secret}</code></> : <><Info size={14} /> Por segurança, os segredos existentes ficam mascarados. Para trocar um segredo, remova o webhook e crie outro.</>}</div><div className="list-stack" style={{ marginTop: 20 }}>{papiWebhooks.length === 0 ? <div className="muted">Nenhum webhook criado ainda.</div> : papiWebhooks.map((webhook) => <article className="surface" style={{ padding: 16 }} key={webhook.id}><div className="profile-field"><span><strong>{webhook.name}</strong><small className="muted">Instância: {webhook.instanceId}</small></span><div style={{ display: "flex", gap: 8, alignItems: "center" }}>{papiConfig?.defaultWebhookId === webhook.id ? <StatusBadge tone="green">Padrão de saída</StatusBadge> : <button type="button" className="btn-secondary" disabled={setDefaultPapiWebhookMutation.isPending} onClick={() => setDefaultPapiWebhookMutation.mutate({ id: webhook.id })}>Usar nos envios</button>}<button type="button" className="btn-secondary" disabled={deletePapiWebhookMutation.isPending} onClick={() => { if (window.confirm(`Remover o webhook ${webhook.name}?`)) deletePapiWebhookMutation.mutate({ id: webhook.id }); }}>Remover</button></div></div><div className="profile-field" style={{ marginTop: 12 }}><span>URL para colar na PAPI</span><button type="button" className="btn-secondary" onClick={() => copyValue(`url-${webhook.id}`, webhook.webhookUrl)}><Copy size={13} />{copied === `url-${webhook.id}` ? "Copiado" : "Copiar URL"}</button></div><code style={{ wordBreak: "break-all" }}>{webhook.webhookUrl}</code><div className="profile-field" style={{ marginTop: 12 }}><span>Header <code>X-PAPI-Webhook-Secret</code></span><code>{webhook.secretMasked}</code></div></article>)}</div><div className="demo-banner" style={{ marginTop: 16, marginBottom: 0 }}><Info size={14} /> O webhook é somente para entrada. O envio de mensagens continua direto para a PAPI, usando o <code>instanceId</code> da mensagem ou o webhook marcado como padrão.</div></section><div className="integration-grid" style={{ marginTop: 20 }}>{integrations.map((integration) => { const tone = integration.status === "connected" ? "green" : integration.status === "pending" ? "amber" : "red"; const Icon = integration.key === "papi" ? MessageCircle : integration.key === "agent" ? Sparkles : integration.key === "crm" ? ContactRoundIcon : CalendarCheck2; return <article className="surface surface-hover integration-card" key={integration.key}><div className="integration-top"><div className="integration-name"><div className="integration-icon"><Icon size={16} /></div><div><strong>{integration.name}</strong><small>{integration.description}</small></div></div><StatusBadge tone={tone}>{integration.status === "connected" ? "Conectado" : integration.status === "pending" ? "Pendente" : "Desconectado"}</StatusBadge></div><p>{integration.detail}</p><div className="integration-footer"><small>Estado derivado da configuração do servidor</small><button className="btn-secondary" onClick={() => window.alert("Configure as variáveis desta integração no Compose e reinicie o serviço.")}><Settings2 size={12} /> Configurar</button></div></article>; })}</div><div style={{ marginTop: 28 }}><SectionTitle eyebrow="Proteções ativas" title="Canais protegidos" /><div className="surface" style={{ padding: 20, display: "grid", gap: 14 }}><div className="profile-field"><span><ShieldCheck size={14} style={{ verticalAlign: "middle", marginRight: 8 }} />PAPI real</span><strong className="green">Protegida</strong></div><div className="profile-field"><span><ShieldCheck size={14} style={{ verticalAlign: "middle", marginRight: 8 }} />Meta Cloud API real</span><strong className="green">Protegida</strong></div><div className="profile-field"><span><ShieldCheck size={14} style={{ verticalAlign: "middle", marginRight: 8 }} />Agente nativo</span><strong className="green">Protegido</strong></div></div></div></PanelLayout>;
}
