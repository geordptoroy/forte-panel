...
function DemoBanner() {
  return <div className="demo-banner"><Info size={15} /><span><strong>Modo demonstração:</strong> os dados são seed local e nenhuma ação externa é enviada para PAPI, CRM ou Agenda.</span></div>;
}
...
export function AgendaPage() {
  ...
  return <PanelLayout eyebrow="Operação / Agenda" title="Agenda" description="Veja seus próximos atendimentos e disponibilidade." actions={<button className="btn-primary"><Plus size={13} /> Novo agendamento</button>}>
    <DemoBanner />
    <div className="agenda-layout"><section className="surface calendar-panel"><div className="calendar-toolbar"><div><span className="eyebrow">Setembro 2026</span><strong>21 — 27 de setembro</strong></div><ViewToggle active={view} onChange={setView} /></div>{view === "dia" ? <div style={{ paddingTop: 18 }}><SectionTitle eyebrow="Sábado, 26 de setembro" title="Agenda do dia" /><div className="list-stack">{appointments.filter((appointment) => appointment.date === "26/09/2026").map((appointment) => { const contact = getContact(appointment.contactId); return <div className="appointment-row" key={appointment.id}><div className="time-block">{appointment.time}</div><div className="row-copy"><strong>{contact.name}</strong><small>{appointment.service} · {appointment.duration}</small><small>{appointment.notes}</small></div><StatusBadge tone={appointment.status === "Confirmado" ? "green" : "amber"}>{appointment.status}</StatusBadge></div>; })}</div></div> : <><div className="week-grid"><div></div>{days.map((day, index) => <div className="day-name" key={day}>{day}<strong>{21 + index}</strong></div>)}{["09:00", "10:00", "11:00", "12:00", "13:00", "14:00", "15:00", "16:00", "17:00"].map((time) => <div key={time} className="time-label">{time}</div>)}{Array.from({ length: 7 * 9 }).map((_, index) => { const day = index % 7; const timeIndex = Math.floor(index / 7); const event = (day === 3 && timeIndex === 5) ? appointments[0] : (day === 5 && timeIndex === 8) ? appointments[1] : null; return <div key={`${day}-${timeIndex}`} className={event ? "calendar-event" : ""}>{event && <><strong>{event.service}</strong><small>{event.time} · {getContact(event.contactId).name.split(" ")[0]}</small></>}</div>; })}</div></>}</section><aside className="surface side-list"><h3>Próximos agendamentos</h3>{appointments.map((appointment) => { const contact = getContact(appointment.contactId); return <div className="appointment-row" key={appointment.id}><div className="time-block">{appointment.date.slice(0, 5)}<small style={{ display: "block", marginTop: 4, color: "#555" }}>{appointment.time}</small></div><div className="row-copy"><strong>{contact.name}</strong><small>{appointment.service}</small></div></div>; })}<button className="btn-secondary" style={{ width: "100%", marginTop: 14 }}>Ver todos os horários</button></aside></div>
  </PanelLayout>;
}
