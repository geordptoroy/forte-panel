...
export function IntegrationsPage() {
  ...
  const createPapiWebhookMutation = trpc.workspace.createPapiWebhook.useMutation({ onSuccess: async (created) => { if (created.secret) setNewWebhookSecret({ name: created.name, secret: created.secret }); setInstanceId(""); setWebhookName(""); await papiConfigQuery.refetch(); }, });
  const setDefaultPapiWebhookMutation = trpc.workspace.setDefaultPapiWebhook.useMutation({ onSuccess: async () => { await papiConfigQuery.refetch(); } });
  const deletePapiWebhookMutation = trpc.workspace.deletePapiWebhook.useMutation({ onSuccess: async () => { await papiConfigQuery.refetch(); } });
