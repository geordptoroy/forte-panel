...
export function TeamPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", password: "", role: "agent" as "admin" | "manager" | "agent", operationalRole: "human_attendant" as "human_attendant" | "ai_attendant" | "professional", professionalId: "" });
  const workspaceQuery = trpc.workspace.current.useQuery();
  const membersQuery = trpc.workspace.members.useQuery();
  const professionalsQuery = trpc.workspace.professionals.useQuery();
  const createMember = trpc.workspace.createMember.useMutation({ onSuccess: () => { setShowInvite(false); setForm({ name: "", email: "", password: "", role: "agent", operationalRole: "human_attendant", professionalId: "" }); membersQuery.refetch(); }, onError: (error) => window.alert(error.message) });
  const createProfessional = trpc.workspace.createProfessional.useMutation({ onSuccess: () => { setForm((current) => ({ ...current, name: "" })); professionalsQuery.refetch(); }, onError: (error) => window.alert(error.message) });
  ...
  return <PanelLayout eyebrow="Sistema / Acessos" title="Equipe" description="Controle quem atende, gerencia e administra este workspace." actions={<button className="btn-primary" onClick={() => setShowInvite((value) => !value)}><Plus size={13} /> Convidar membro</button>}>
    ...
    {showInvite && <section className="surface team-invite-panel"><SectionTitle eyebrow="Novo acesso" title="Criar acesso operacional" /><div className="form-grid"><div className="form-field"><label>Nome</label><input className="input-control" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Nome do colaborador" /></div><div className="form-field"><label>E-mail</label><input className="input-control" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="colaborador@empresa.com" type="email" /></div><div className="form-field"><label>Senha inicial</label><input className="input-control" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} placeholder="mínimo 8 caracteres" type="password" /></div><div className="form-field"><label>Tipo de colaborador</label><select className="select-control" value={form.operationalRole} onChange={(event) => setForm({ ...form, operationalRole: event.target.value as typeof form.operationalRole })}><option value="human_attendant">Atendente humano</option><option value="ai_attendant">Atendente IA</option><option value="professional">Profissional executor</option></select></div><div className="form-field"><label>Papel administrativo</label><select className="select-control" value={form.role} onChange={(event) => setForm({ ...form, role: event.target.value as typeof form.role })}><option value="agent">Atendente</option><option value="manager">Gerente</option><option value="admin">Administrador</option></select></div>{form.operationalRole === "professional" && <div className="form-field"><label>Profissional vinculado</label><select className="select-control" value={form.professionalId} onChange={(event) => setForm({ ...form, professionalId: event.target.value })}><option value="">Selecione</option>{professionals.map((professional) => <option key={professional.id} value={professional.id}>{professional.name}</option>)}</select></div>}</div><button className="btn-primary" style={{ marginTop: 15 }} disabled={createMember.isPending || !form.name || !form.email || form.password.length < 8 || (form.operationalRole === "professional" && !form.professionalId)} onClick={() => createMember.mutate({ ...form, professionalId: form.professionalId ? Number(form.professionalId) : undefined })}><Plus size={13} /> Criar acesso</button></section>}
    <section className="surface team-invite-panel"><SectionTitle eyebrow="Cadastro operacional" title="Profissionais executores" /><p className="muted">Cadastre quem realiza o serviço. Depois vincule o profissional ao login no formulário acima.</p><div className="form-grid"><div className="form-field"><label>Nome do profissional</label><input className="input-control" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Ex.: Gabriel Ferreira" /></div><div className="form-field"><label>Especialidade</label><input className="input-control" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="Ex.: Eletricista" /></div></div><button className="btn-secondary" style={{ marginTop: 15 }} disabled={createProfessional.isPending || form.name.length < 2} onClick={() => createProfessional.mutate({ name: form.name, specialty: form.email })}><Plus size={13} /> Cadastrar profissional</button><div className="team-table" style={{ marginTop: 16 }}>{professionals.map((professional) => <div className="team-row" key={professional.id}><div className="avatar">{professional.name.slice(0, 2).toUpperCase()}</div><div className="row-copy"><strong>{professional.name}</strong><small>{professional.specialty ?? "Especialidade não informada"}</small></div><span className="team-role">Executor</span><StatusBadge tone="green">Ativo</StatusBadge></div>)}</div></section>
...
export function ProfessionalPortalPage() {
  ...
  const professional = agendaQuery.data?.professionals?.[0];
  return <PanelLayout eyebrow="Minha operação" title="Meu trabalho" description="Sua agenda de hoje e os próximos atendimentos atribuídos a você."><div className="demo-banner operational-banner"><ClipboardList size={15} /><span><strong>{professional?.name ?? (accessQuery.data?.professionalId ? "Profissional vinculado" : "Acesso administrativo")}</strong> · esta visão é filtrada pelo vínculo profissional do login.</span></div><section className="surface team-table-card"><SectionTitle eyebrow="Agenda atribuída" title={`${appointments.length} atendimento${appointments.length === 1 ? "" : "s"}`} action={<StatusBadge tone="green">Agenda nativa</StatusBadge>} /><div className="team-table">{appointments.length ? appointments.map((appointment) => <div className="team-row" key={appointment.id}><div className="time-block">{new Date(appointment.startsAt).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</div><div className="row-copy"><strong>{appointment.serviceName ?? `Atendimento #${appointment.id}`}</strong><small>{new Date(appointment.startsAt).toLocaleDateString("pt-BR")} · {appointment.contactName ?? "Cliente não informado"} · {appointment.notes ?? "Sem observações"}</small></div><StatusBadge tone={appointment.status === "confirmed" ? "green" : appointment.status === "completed" ? "neutral" : "amber"}>{appointment.status}</StatusBadge></div>) : <EmptyState icon={ClipboardList} title="Nenhum atendimento atribuído" description="Quando a agenda receber um atendimento para este profissional, ele aparecerá aqui." />}</div></section></PanelLayout>;
}
