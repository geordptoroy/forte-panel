...
O pacote enviado é `n8n-nodes-forte-panel@0.1.0`. Ele declara o node como `usableAsTool: true`, portanto o node pode ser conectado diretamente à porta **Tools** do `AI Agent`.

No node `Forte Panel1` do workflow anexado, configure a credencial **Forte Panel API** com:

| Campo | Valor dentro da rede Docker |
|---|---|
| Base URL | `http://forte-panel:3000/api/v1` |
| API Key | o mesmo valor de `FORTE_API_KEY` do container Panel |

No compose atual, o valor de teste é `teste-panel-local`. Em produção, use uma chave forte e exclusiva.

A conexão correta é:

```text
Forte Panel1 --(AI Tool)--> AI Agent
```

Não conecte a saída principal da tool ao fluxo de mensagens. O node deve permanecer ligado somente ao conector **Tools** do agente.

## Instalação do pacote no n8n

Como o n8n está em Docker, o pacote precisa estar instalado no ambiente do n8n, não apenas no container do Panel. Se o node já aparece no editor como `Forte Panel`, ele está carregado e basta configurar a credencial.

Se aparecer `Unknown node type`, instale o `.tgz` dentro de uma imagem customizada do n8n ou por Community Nodes e reinicie o container. Depois verifique se o node aparece na busca por “Forte Panel”.

Após instalar ou atualizar o pacote, reinicie o n8n:

```powershell
docker compose -p evomain -f .\docker-compose.yaml restart n8n
```

## Teste seguro

1. Salve a credencial no node `Forte Panel1`.
2. Confirme a conexão laranja **AI Tool** com o `AI Agent`.
3. Ative o workflow.
4. Envie uma mensagem de teste informando serviço e localização.
5. Verifique o lead no Panel.
6. Teste uma consulta de disponibilidade antes de testar a criação de um agendamento.
7. Não habilite `queue_message` enquanto os nós de envio atuais ainda forem responsáveis pela resposta normal.

Logs úteis:

```powershell
docker compose -p evomain -f .\docker-compose.yaml logs -f forte-panel
```

```powershell
docker compose -p evomain -f .\docker-compose.yaml logs -f forte-panel-worker
```

> **Importante:** se a resposta normal continuar saindo pelos nós PAPI do workflow, não use `queue_message` para essa mesma resposta. Para migrar o envio completamente para o worker do Panel, primeiro remova ou desative o caminho PAPI correspondente e teste em um fluxo separado.

## Referências

[1]: https://github.com/geordptoroy/forte-panel/blob/main/API_CONTRACT.md "Contrato da API Forte Panel"
[2]: https://github.com/geordptoroy/forte-panel/tree/main/packages/n8n-nodes-forte-panel "Community node Forte Panel para n8n"
